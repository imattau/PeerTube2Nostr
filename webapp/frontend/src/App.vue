...
import { PlusCircle, Rss, Trash2, CheckCircle2, Play, ExternalLink, Clock, Database, ShieldCheck } from 'lucide-vue-next'
...
<template>
  <AppShell>
            ...
            <div class="p-4">
              <div v-if="store.sources.length > 0" class="divide-y divide-border-subtle">
                <div v-for="source in store.sources" :key="source.id" class="py-2 flex items-center justify-between group">
                  ...
                  <div class="opacity-0 group-hover:opacity-100"><Button variant="ghost" size="sm" @click="store.deleteSource(source.id)"><Trash2 class="w-3.5 h-3.5" /></Button></div>
                </div>
              </div>
    ...
    <BaseModal :show="modal.show" :title="modal.title" @close="modal.show = false">
      <div class="space-y-5">
        <input v-model="modal.value" @keyup.enter="handleModalSubmit" autofocus class="w-full bg-bg border border-border-subtle rounded-md px-4 py-2.5 text-text-primary focus:outline-none focus:ring-1 focus:ring-accent" />
