...
async def verify_api_key(request: Request):
    # Use the key from environment or the one generated by manager
    target_key = manager.api_key or API_KEY
    if not target_key:
        return
    auth_header = request.headers.get("X-API-Key")
    if auth_header != target_key:
...
async def get_setup_status():
    return {
        ...
        "has_api_key": bool(manager.api_key or API_KEY)
    }
...
@app.post("/api/setup/config", dependencies=[Depends(verify_api_key)])
async def set_setup_config(data: SetupConfig):
    manager.set_signing_config(data.method, data.nsec, data.bunker_url, data.pubkey)
    return {"status": "ok"}

@app.post("/api/setup/complete", dependencies=[Depends(verify_api_key)])
